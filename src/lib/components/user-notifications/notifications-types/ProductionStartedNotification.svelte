<script lang="ts">
  ...
  import {
    ...
    type ProductionStartedNotificationPayload,
  ...
  export let notification: InternalUserNotification<ProductionStartedNotificationPayload>
  export let onActionClick: (options: UserNotificationActionPayload) => void = () => {}

  const count: number = notification.metadata?.quantity || 0
...
<UserNotification
  ...
  <svelte:fragment slot="content">
    <NotificationText {notification}>
      {#if count && product}
        {$_('Notification A Production Of Number Of Products Was Started', {
          values: {
            count,
            product,
