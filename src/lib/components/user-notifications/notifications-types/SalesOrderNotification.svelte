...
<UserNotification {notification}>
      ...
      {#if orderMnemonicId && notification.metadata.id}
        <div>
          ...
          <a
            ...
            href={upsertSalesOrderRoute({ orderId: notification.metadata.id, useReferrer: true })}>
            {orderMnemonicId}
          </a>
        </div>
      {/if}
  ...
  <svelte:fragment slot="actions">
    {#if (notification.type === 'SALES_ORDER_CREATED' || notification.type === 'SALES_ORDER_ACCEPTED') && notification.metadata.id}
      <IconButton
        variant="ghost"
