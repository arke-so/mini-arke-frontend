 ...
 */
export function getQueryFromURL<T = AdditionalQueryProps>(url: URL, overrides: T = {} as T): ExtendedFilterQuery {
 ...
 * @returns object {ExtendedFilterQueryObject} - A FilterQueryObject with optional properties if any are passed
 */
export function queryStoreToRequestObject({ search, query, page, hasMore, ...props }: InfiniteQueryStore) {
  return createQueryRequestObject({
    search,
    query,
    limit: DEFAULT_ITEMS_LIMIT,
    offset: page * DEFAULT_ITEMS_LIMIT,
    ...props,
  })
}
